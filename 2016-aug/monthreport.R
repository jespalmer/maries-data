# file - a csv file month1 - the current month number formatted as '01',...,'12',
# default is 01 for Jan month2 - the previous month number formatted as
# '01',...,'12', default is 01 for Jan year1 - the current year formatted as
# '16', default is 16 for 2016 year2 - the previous year formatted as '15',
# default is 15 for 2015
...
report <- function(file, month1 = "01", month2 = "01", year1 = "16", year2 = "15") {
    ...
    if (month1 == "03") {
        ...
        month_lastyear <- subset(this_month, format.Date(placed_date, "%m") %in%
            gooddates & format.Date(placed_date, "%y") == year2)

        month_thisyear <- subset(this_month, format.Date(placed_date, "%m") %in%
            gooddates & format.Date(placed_date, "%y") == year1)

        # } else if (month1 == '06') { date <- 4:6 for (d in date < 10) { gooddates <-
        # lapply(date, function(n) sprintf('%02d', n)) } # get the months and years we
        # want month_lastyear <- subset(this_month, format.Date(placed_date, '%m') %in%
        # gooddates & format.Date(placed_date, '%y') == year2) month_thisyear <-
        # subset(this_month, format.Date(placed_date, '%m') %in% gooddates &
        # format.Date(placed_date, '%y') == year1) } else if (month1 == '09') { date <-
        # 7:9 for (d in date < 10) { gooddates <- lapply(date, function(n)
        # sprintf('%02d', n)) } # get the months and years we want month_lastyear <-
        # subset(this_month, format.Date(placed_date, '%m') %in% gooddates &
        # format.Date(placed_date, '%y') == year2) month_thisyear <- subset(this_month,
        # format.Date(placed_date, '%m') %in% gooddates & format.Date(placed_date, '%y')
        # == year1) } else if (month1 == '12') { date <- 10:12 for (d in date < 10) {
        # gooddates <- lapply(date, function(n) sprintf('%02d', n)) } # get the months
        # and years we want month_lastyear <- subset(this_month, format.Date(placed_date,
        # '%m') %in% gooddates & format.Date(placed_date, '%y') == year2) month_thisyear
        # <- subset(this_month, format.Date(placed_date, '%m') %in% gooddates &
        # format.Date(placed_date, '%y') == year1)

    } else {
        # get the months and years we want
        month_lastyear <- subset(this_month, format.Date(placed_date, "%m") == month2 &
            format.Date(placed_date, "%y") == year2)

        month_thisyear <- subset(this_month, format.Date(placed_date, "%m") == month1 &
            format.Date(placed_date, "%y") == year1)
    }
    ...
    ytd_thisyear <- subset(this_month, format.Date(placed_date, "%y") == year1)

    ytd_lastyear <- subset(this_month, format.Date(placed_date, "%y") == year2)
    ...
    s <- ggplot(no_outlier, aes(x = year, y = total, color = product, group = product)) +
        scale_fill_manual(values = colors) + geom_line() + geom_point(shape = 1) +
        ggtitle("Total Month Gross Sales Compared without Cinnamon Rolls") + xlab("Year") +
        ylab("Total Month Sales by Product in Dollars") + scale_y_continuous(breaks = seq(0,
        800, 25), labels = scales::dollar)
    s <- s + theme(plot.title = element_text(color = "black", face = "bold", size = 12,
        hjust = -0.2, vjust = 1), legend.text = element_text(size = 3), legend.position = "right",
        legend.key = element_rect(fill = "white"), legend.background = element_rect(fill = NA),
        legend.title = element_blank())
    s <- s + guides(col = guide_legend(ncol = 4, byrow = TRUE, title.hjust = 0.4))
    # save as pdf
    pdf(paste("aug_summary_byproduct", ".pdf", sep = ""), useDingbats = FALSE)
    print(s)
    dev.off()

    # create plot for total comparsion for whole month totals
    par(default_par)
    g <- ggplot(data = long, aes(x = year, y = total, fill = year)) + geom_bar(stat = "identity") +
        guides(fill = FALSE) + xlab("Year") + ylab("Total Month Sales in Dollars") +
        ggtitle("Total Month Gross Sales Compared") + scale_y_continuous(labels = scales::dollar)
    # save as pdf
    pdf(paste("aug_summary_totals", ".pdf", sep = ""), useDingbats = FALSE)
    print(g)
    dev.off()
    ...
    product.graph <- function(long, ...) {
        ...
        for (i in seq_along(product_list)) {
            ...
            plot <- ggplot(subset(long, long$product == product_list[i]), aes(x = year,
                y = total, fill = year)) + geom_bar(stat = "identity") + facet_wrap(~product,
                scales = "free") + guides(fill = FALSE) + xlab("Year") + ylab("Product's monthly gross profit in dollars") +
                ggtitle(paste("August ", product_list[i], " sales in 2015 and 2016",
                  sep = "")) + scale_y_continuous(labels = scales::dollar)
            # save plots as .pdf
    ...
    compare_months <- compare_months[order(-compare_months$currentyear, compare_months$product),
        ]
    ...
    top_products <- top_products[order(-top_products$total, top_products$product),
        ]

    # create plot for total comparsion for whole month totals
    par(new_par)
    g <- ggplot(data = top_products, aes(x = year, y = total, fill = year)) + geom_bar(stat = "identity") +
        guides(fill = FALSE) + facet_wrap(~product, ncol = 5, scales = "free") +
        xlab("Year") + ylab("Total Month Sales in Dollars") + ggtitle("Total Month Gross Sales for Bestselling Products This Year Compared") +
        scale_y_continuous(labels = scales::dollar)
    g <- g + theme(plot.title = element_text(color = "black", face = "bold", size = 12,
        hjust = 0.2, vjust = 1), legend.text = element_text(size = 3), legend.title = element_blank(),
        axis.text = element_text(size = 3), axis.title = element_text(size = 3),
        strip.text.x = element_text(size = 3))
    # save as pdf
    pdf(paste("aug_summary_bestproducts", ".pdf", sep = ""), useDingbats = FALSE)
    print(g)
    dev.off()
    ...
    cat("\nTotal gross month sales for bottom 20 products for current year", "\n",
        " as percentage of current year YTD: \n", (sum(worst_products$currentyear))/sum(ytd_thisyear$product_price) *
            100, "%", "\n")
    ...
    worst_products <- worst_products[order(-worst_products$total, worst_products$product),
        ]

    # create plot for total comparsion for whole month totals
    par(new_par)
    g <- ggplot(data = worst_products, aes(x = year, y = total, fill = year)) + geom_bar(stat = "identity") +
        guides(fill = FALSE) + facet_wrap(~product, ncol = 5, scales = "free") +
        xlab("Year") + ylab("Total Month Sales in Dollars") + ggtitle("Total Month Gross Sales for Worstselling Products This Year Compared") +
        scale_y_continuous(labels = scales::dollar)
    g <- g + theme(plot.title = element_text(color = "black", face = "bold", size = 12,
        hjust = 0.2, vjust = 1), legend.text = element_text(size = 3), legend.title = element_blank(),
        axis.text = element_text(size = 3), axis.title = element_text(size = 3),
        strip.text.x = element_text(size = 3))
    # save as pdf
    pdf(paste("aug_summary_worstproducts", ".pdf", sep = ""), useDingbats = FALSE)
    print(g)
    dev.off()
    ...
    lastyear_storesums <- with(month_lastyear, tapply(product_price, name, sum))

    thisyear_storesums <- with(month_thisyear, tapply(product_price, name, sum))
    ...
    compare_storemonths <- compare_storemonths[order(-compare_storemonths$currentyear,
        compare_storemonths$store), ]

    cat("\nTotal month gross sales in dollars for each store compared:\n")
    compare_stores_ordered <- compare_storemonths[order(-compare_storemonths$currentyear),
        ]
    ...
    par(new_par)
    t <- ggplot(data = top_stores, aes(x = year, y = total, fill = year)) + geom_bar(stat = "identity") +
        guides(fill = FALSE) + facet_wrap(~store, ncol = 5, scales = "free") + xlab("Year") +
        ylab("Total Month Sales in Dollars") + ggtitle("Total Month Gross Sales for Bestselling Stores This Year Compared") +
        scale_y_continuous(labels = scales::dollar)
    t <- t + theme(plot.title = element_text(color = "black", face = "bold", size = 12,
        hjust = 0.2, vjust = 1), legend.text = element_text(size = 3), legend.title = element_blank(),
        axis.text = element_text(size = 3), axis.title = element_text(size = 3),
        strip.text.x = element_text(size = 3))
    # save as pdf
    pdf(paste("aug_summary_beststores", ".pdf", sep = ""), useDingbats = FALSE)
    print(t)
    dev.off()
    ...
    cat("\nTotal gross month sales in dollars for bottom 20 stores for current year",
        "\n", " as percentage of current year YTD: \n", (sum(worst_stores$currentyear))/sum(ytd_thisyear$product_price) *
            100, "%", "\n")
    ...
    # create plot for total comparsion for store whole month totals
    par(new_par)
    t <- ggplot(data = worst_stores, aes(x = year, y = total, fill = year)) + geom_bar(stat = "identity") +
        guides(fill = FALSE) + facet_wrap(~store, ncol = 5, scales = "free") + xlab("Year") +
        ylab("Total Month Sales in Dollars") + ggtitle("Total Month Gross Sales for Wostselling Stores This Year Compared") +
        scale_y_continuous(labels = scales::dollar)
    t <- t + theme(plot.title = element_text(color = "black", face = "bold", size = 12,
        hjust = 0.2, vjust = 1), legend.text = element_text(size = 3), legend.title = element_blank(),
        axis.text = element_text(size = 3), axis.title = element_text(size = 3),
        strip.text.x = element_text(size = 3))
    # save as pdf
    pdf(paste("aug_summary_worststores", ".pdf", sep = ""), useDingbats = FALSE)
    print(t)
    dev.off()
    ...
    compare_storesprofit <- subset(compare_storesprofit, lastyear > 0 & currentyear >
        0)
    compare_storesprofit <- compare_storesprofit[order(-compare_storesprofit$currentyear),
        ]
    ...
    par(default_par)
    y <- ggplot(data = both_profit, aes(x = year, y = total, fill = year)) + geom_bar(stat = "identity") +
        guides(fill = FALSE) + facet_wrap(~store, ncol = 5, scales = "free") + xlab("Year") +
        ylab("Total Month Sales in Dollars") + ggtitle("Total Month Gross Sales in Dollars for Stores That Made Profit in Both Years Compared") +
        scale_y_continuous(labels = scales::dollar)
    y <- y + theme(plot.title = element_text(color = "black", face = "bold", size = 12,
        hjust = 0.2, vjust = 1), legend.text = element_text(size = 3), legend.title = element_blank(),
        axis.text = element_text(size = 3), axis.title = element_text(size = 3),
        strip.text.x = element_text(size = 3))
    # save as pdf
    pdf(paste("aug_summary_profitstores", ".pdf", sep = ""), useDingbats = FALSE)
    print(y)
    dev.off()
}
